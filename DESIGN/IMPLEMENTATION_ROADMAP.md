...
| 7 Screens | STARTED (Connect reference + Settings; pull-based) | **Connect Screen is fully DS-native** (PcButton + PcCard + PcTextField + PcDialog + tokens; no legacy widget left) and is the reference. **Settings panel migrated next** (DESIGN_FEEDBACK #2): 17 inputs -> dense-numeric PcTextField (which grew `keyboardType`/`textAlign`/`dense`/optional-label additively for it), Apply -> PcButton. Strategy is now **pull-based**: components are extended/built only when a real screen demands them. Remaining (menu, lobby, game, finished, rules, spectate) still ad-hoc. |
