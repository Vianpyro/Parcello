...
| 1 Tokens | DONE (spacing + colours) | spacing scale (`Pc.s2..s24` + `cardInset`) in `lib/tokens.dart`; ~88 value-preserving replacements across 19 widget files (pixel-identical); every on-grid single-value literal migrated. Off-grid one-offs (3/5/10/18/20), structural 0/1, and multi-value bespoke insets (`fromLTRB`/mixed `symmetric`) left literal BY POLICY (no token exists / bespoke geometry). One raw colour closed (`hairlineGold`). **C2 guard** (`test/design_c2_guard_test.dart`) enforces colours + on-grid spacing in ERROR mode, verified to catch injected violations. Durations remain a review-only C2 concern (animation vs behavioural can't be told apart mechanically). Remaining/optional: an elevation helper if a use case appears; align off-grid one-offs to grid in a future visual-review pass |
| 2 Typography | IN PROGRESS (role-mappable done) | `lib/typography.dart` (`PcText` roles, DDR-0018). ~27 sites migrated to roles value-preservingly (caption x11, whisper, label, body, tileTitle, wordmark). The **family rule** (DDR-0018/TYPOGRAPHY.md, owner-set): Fraunces = brand only, Inter = default UI. Applied: connect title -> `wordmark` (Fraunces, was inline Inter); rules-page section headings Fraunces -> Inter (flagged - revert if they were meant as "emblematic"). **C2 guard extended** to enforce inline `fontFamily: 'Fraunces'` (must be `PcText.wordmark`), verified live. **A3 DONE (2026-07):** the theme's default text ink is now explicit `Pc.text` (`main.dart`, `textTheme.apply(bodyColor/displayColor)`), so a bare/size-only style inherits the DS colour. The size-ONLY-at-a-role-size sites (8: 7x `fontSize:12`->`label`, 1x `14/w700`->`rowTitle`) migrated value-preservingly, and the **`fontSize` C2 guard is now ENFORCED** (a `TextStyle(fontSize: N)` at a role size 10-18 must be the role; verified live). What remains inline is genuinely bespoke and NOT flagged: off-role display sizes (15/20/26/32), computed sizes (`size*0.5`/`nameSize`/`metaSize`), and role-size styles carrying a weight/explicit colour/letterSpacing a role does not (gold/goldDark/parchmentInk headers, w600/w800 emphasis, tabular amounts - already correct). |
| 3 Theme | PARTIAL (text ink done) | base ThemeData exists; **default text colour now explicit `Pc.text`** (A3). Remaining consolidation (input/dialog/chip themes, deleting local styles Phase 5 proves) not yet done. |
| 4 Anim primitives | PARTIAL | engine done; marker/pulse/re-orient missing; `Motion.stateFade` (200ms UI state transition) added additively for SeatTile (#4) |
| 5 Core widgets | IN PROGRESS | inventory + dependency order in DESIGN/COMPONENT_INVENTORY.md; **Design Showcase** (`lib/ui/showcase/`, debug-only) covering per-section a11y edge cases (text zoom, narrow width) + a keyboard/focus section + a running a11y-coverage note. Landed, API frozen (DDR-0019), demoed + tested: **PcButton**, **PcCard** (flat), **PcTextField** (hairline underline, gold focus), **PcDialog** (raised confirm/prompt), **PcChip** (tap-to-order, gold-selected), **SeatTile** (domain, L3). **Selection is now blocker-driven** (DESIGN_FEEDBACK #5): build the component whose absence blocks the NEXT screen's clean migration, max reuse as tie-break - PcChip was pulled because the Legal Route builder + mod picker chips had no DS equivalent (deferred no longer). Still deferred: PcHairline, PcBadge (no screen demands them value-preservingly). Adoption incremental; `Card`->`PcCard` also drops the stray Material shadow. |
| 6 Composite / domain (incl. AuctionWidget) | STARTED (SeatTile) | SeatTile shipped (L3, CAR-0001). **CAR GATE (owner):** every L3/L4 domain component now needs a ratified Component Architecture Record (`DESIGN/car/`) BEFORE implementation - responsibility/boundaries/API/invariants/Motion/A11y/l10n validated first. Pull-based picks WHICH is next; the CAR is the gate before coding. AuctionWidget stays highest risk / the #1 UX gap (and the highest-stakes CAR). |
| 7 Screens | STARTED (Connect + Settings + Lobby + Game HUD; pull-based) | **Connect** fully DS-native (reference). **Settings** (#2): dense-numeric PcTextField (grew additively). **Lobby / side panel** (#3): Card->PcCard, wideButton->PcButton, resign->PcDialog (grew `destructive`). **Game HUD** (#4): the seat rows became **`SeatTile`** - the first BUSINESS/domain component, presentational (formatted strings + chit anchor + bid slot), built WITHOUT PcBadge/MoneyChit to stay value-preserving. **Game HUD chips** (#5): the Legal Route builder + menu mod-picker chips became **`PcChip`** (blocker-driven pull, 2 consumers). Selection rule is now blocker-driven (build the component that blocks the next screen; max reuse as tie-break). Domain components emerge from their surfaces (SeatTile done; PropertyCard/MoneyChit/TradeOfferCard/AuctionWidget pending). Gameplay & Visual-Debt registers tracked in DESIGN_FEEDBACK. ActionsPanel follow-on (not a new component): `PcTextField.inputFormatters` for bid/bribe, then the action-button sizing decision. Legacy remaining: menu chrome, rest of HUD, board, trades, feedback, rules. |
