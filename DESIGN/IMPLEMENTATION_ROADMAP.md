...
| 7 Screens | STARTED (Connect + Settings + Lobby; pull-based) | **Connect** fully DS-native (reference). **Settings** (DESIGN_FEEDBACK #2): 17 inputs -> dense-numeric PcTextField (grew additively), Apply -> PcButton. **Lobby / side panel** (#3): 5 Card -> PcCard, 7 wideButton -> PcButton, resign confirm -> PcDialog (which grew `destructive` additively). Strategy is **pull-based**: components extended/built only on real demand - the lobby deliberately did NOT build PcBadge/PcListRow (the seat tags are text, the seat rows are the future SeatTile, #13; the trades are TradeOfferCard, #14 - both L3, still legacy). Legacy remaining lives in menu, game HUD, board, trades, feedback, rules (see DESIGN_FEEDBACK Coverage table). |
