...
| 2 Typography | IN PROGRESS | `lib/typography.dart` (`PcText` roles) landed per DDR-0018 (size+weight+family + default colour; some size-inheriting); menu wordmark/tile/replay-tips migrated as value-preserving proof (51 tests green). Flagged for owner intent (NOT mechanical): the connect-screen title renders in Inter while the menu wordmark is Fraunces (visual-identity says Fraunces) - a real inconsistency to align deliberately; several one-off sizes (15/18/20) and inherited-size bodies stay bespoke until a role clearly fits. Remaining: migrate the other ~19 files incrementally; extend the C2 guard to `fontSize` once coverage is high (warning->error, as spacing) |
