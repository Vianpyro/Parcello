...
group-scaled, a modern reinterpretation replacing the original "resorts"
idea) instead of four stations, two chance tiles, one tax tile, slightly
less starting cash); the clients render any `4*(d-1)` square ring. The
old no-op Free Parking corner is now the Exposition, a property-spotlight
...
| Stations (gares) | 4 group-scaled tiles | removed, or repurposed | DONE - 2 "utility" tiles on `base` (Wi-Fi, The Chatbot), a later reinterpretation of the original "resorts" idea |
